...
void Analyzer::Fit(){  
	...
	f2->SetParameter(0,20410); // D = DISKRIMINATOR
	f2->SetParameter(1,829.9); // SIRINA=GAMMA
	f2->SetParameter(2,124.4); //M = MASA
	fsum=new TF1 ("fsum", "[0]+[1]*x+[2]*x*x+([3]*[4])/((x*x-[5]*[5])*(x*x-[5]*[5])+0.25*[4]*[4])",110,150);
	fsum->SetParameter(0,51.81);
	fsum->SetParName(0,"A");
	fsum->SetParameter(1,-0.6214);
	fsum->SetParName(1,"B");
	fsum->SetParameter(2,0.002451);
	fsum->SetParName(2,"C");
